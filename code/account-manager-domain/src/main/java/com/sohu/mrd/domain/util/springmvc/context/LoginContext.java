...
public class LoginContext {
	 ...
	 */
	public static final String HTTP_LOGIN_CONTEXT = "com.sohu.mrd.domain.util.springmvc.context.LoginContext";
	/**
	 * 登录时写入的对象
	 */
	public static final String HTTP_DOTNET_LOGIN_TICKET_CONTEXT = "com.sohu.mrd.domain.util.springmvc.context.FormsAuthenticationTicket";
	/**
	 * 用户ID
	 */
	private long userId;
	...
	public static void setLoginContext(LoginContext loginContext) {
		...
		ServletContext servletContext = webApplicationContext
				.getServletContext();
		servletContext.setAttribute(HTTP_LOGIN_CONTEXT, loginContext);
	}
	...
	public static LoginContext getLoginContext() {
		...
		ServletContext servletContext = webApplicationContext
				.getServletContext();
		return (LoginContext) servletContext.getAttribute(HTTP_LOGIN_CONTEXT);
	}
	...
	public static void remove() {
		...
		ServletContext servletContext = webApplicationContext
				.getServletContext();
		servletContext.removeAttribute(HTTP_LOGIN_CONTEXT);
	}
	 ...
	 *
	 * @param value
	 *            需要反向构造的串。形式如下：userId=123,pin=yangsy,nick=杨思勇
	 * @return 上下文
	 * @see #toCookieValue()
	 */
						...
						if (StringUtils.isNotBlank(field)) {
							BeanUtils
									.setProperty(
											...
											field,
											StringEscapeUtils
													.unescapeCombinedCookie(keyValue[1]));
						}
					} catch (Exception e) {
	...
	public String toCookieValue() {
		...
		if (StringUtils.isNotEmpty(pin)) {
			sb.append(",pin=").append(
					StringEscapeUtils.escapeCombinedCookie(pin));
		}
		if (StringUtils.isNotEmpty(nick)) {
			sb.append(",nick=").append(
					StringEscapeUtils.escapeCombinedCookie(nick));
		}
		if (created != 0) {
