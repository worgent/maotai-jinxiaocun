...
public class Money implements Serializable, Comparable {
     ...
     *
     * <p>
     * 此处，“厘”是指货币的最小单位，“元”是货币的最常用单位，
     * 不同的币种有不同的元/厘换算比例，如人民币是1000，而日元为1。
     */
     ...
     * <p>
     * 创建一个具有金额<code>amount</code>元和指定币种<code>currency</code>的货币对象。
     * 如果金额不能转换为整数厘，则使用指定的取整模式<code>roundingMode</code>取整。
     *
     * @param amount 金额，以元为单位。
     * @param currency 币种。
    ...
    public Money(BigDecimal amount, Currency currency, int roundingMode) {
        this.currency     = currency;
        this.cent         = rounding(amount.movePointRight(3),
                roundingMode);
    }
    ...
    public BigDecimal getAmount() {
        return BigDecimal.valueOf(this.cent, 3);
    }
     ...
     * 获取本货币币种的元/厘换算比率。
     *
     * @return 本货币币种的元/厘换算比率。
     */
    public int getCentFactor() {
        return centFactors[3];
    }
