...
/**
 * dotnet写入登录session的ticket
 * User: bjzhyb
 * Date: 2010-6-14
 * Time: 21:16:58
 */
public class FormsAuthenticationTicket {
    private final static Logger log = Logger.getLogger(FormsAuthenticationTicket.class);
    /**
     * 指定的版本号
     */
    ...
    public static void setTicket(FormsAuthenticationTicket ticket) {
        ActionContext.getContext().put(LoginContext.HTTP_DOTNET_LOGIN_TICKET_CONTEXT, ticket);
    }
    /**
    ...
    public static void setTicketBefore(String userName,String pwd) {
        try {
            FormsAuthenticationTicket ticket=new FormsAuthenticationTicket(userName,null,"/",null,null,0,false);
            setTicket(ticket);
        } catch (Exception e) {
    ...
    public static FormsAuthenticationTicket getTicket() {
        return (FormsAuthenticationTicket) ActionContext.getContext().get(LoginContext.HTTP_DOTNET_LOGIN_TICKET_CONTEXT);
    }
    ...
    public static void remove() {
        ActionContext.getContext().getContextMap().remove(LoginContext.HTTP_DOTNET_LOGIN_TICKET_CONTEXT);
    }
}