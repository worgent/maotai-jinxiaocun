function testCheck(id,obj){
	$.ajax({
		url: "/monitor/site_validateOfflineTask.action?id=" + id,
		type: "get",
		cache: false,
		beforeSend: function(){
...
function switchCheck(id,obj){
	$.ajax({
		url: "/monitor/site_switchOfflineTask.action?id=" + id,
		type: "get",
		cache: false,
		beforeSend: function(){
		},
...
$(function(){
	setInterval(function(){
		$.ajax({
			url: "/json/offlineListStatus.action",
			type: "get",
			cache: false,
			dataType: "json",
			beforeSend: function(){
			...
			success: function(data){
				var d = data.offlineServiceTaskList;
				for(var index in d){
