...
$.widget("bootstrap.bwizard", {
	...
	_refreshStep: function () {
		...
		if (this.lis) {
			this.lis.removeClass('active latest').attr('aria-selected', false).find('.label').removeClass('badge-inverse');
			if (o.activeIndex >= 0 && o.activeIndex <= this.lis.length - 1) {
				if (this.lis) {
					...
					for(var i=0;i<o.activeIndex;i++){
						this.lis.eq(i).addClass('latest');
					}
				}
			}
		}
